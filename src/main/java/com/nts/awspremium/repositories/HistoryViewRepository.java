...
public interface HistoryViewRepository extends JpaRepository<HistoryView,Long> {
    ...
    @Query(value = "SELECT count(*) FROM AccPremium.historyview where running=1 and orderid in(select orderid from videoview where service in(select service from service where geo='vn' and maxtime<30));",nativeQuery = true)
