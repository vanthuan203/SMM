...
public interface AccountRepository extends JpaRepository<Account,Long> {
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE account SET password=?1,recover=?2,live=?3,encodefinger=?4,cookie=?5,endtrial=?6,endtrialstring=?7,vps='' where username=?8",nativeQuery = true)
    ...
    @Query(value = "SELECT id  FROM account where live=0  order by rand()  limit 1",nativeQuery = true)
