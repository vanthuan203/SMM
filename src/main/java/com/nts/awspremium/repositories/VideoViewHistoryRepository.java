...
public interface VideoViewHistoryRepository extends JpaRepository<VideoViewHistory,Long> {
    ...
    @Query(value = "SELECT * from videoviewhistory where  videoid in (?1) or orderid in (?1) or channelid=?1 order by enddate desc",nativeQuery = true)
    public List<VideoViewHistory> getVideoViewHistoriesByListVideoId(List<String> list_orderid);

    @Query(value = "SELECT * from videoviewhistory where  (videoid in (?1) or orderid in (?1) or channelid=?1) and user=?2 order by enddate desc",nativeQuery = true)
