...
public interface VideoViewHistoryRepository extends JpaRepository<VideoViewHistory,Long> {
    ...
    @Modifying
    @Transactional
    @Query(value = "update videoviewhistory set viewend=?1,timecheckbh=?2 where videoid=?3 and DATE_FORMAT(FROM_UNIXTIME((timestart-3*60*60*1000+24*4*60*60*1000) / 1000), '%Y-%m-%d')=DATE_FORMAT(ADDDATE( UTC_TIMESTAMP(), INTERVAL +7 HOUR), '%Y-%m-%d')",nativeQuery = true)
