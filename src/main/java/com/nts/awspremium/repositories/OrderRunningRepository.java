...
public interface OrderRunningRepository extends JpaRepository<OrderRunning,Long> {
    ...
    public Integer get_Order_By_Order_Key_And_Task_OFF(String order_id,String task);

    @Query(value = "SELECT count(o) from OrderRunning o JOIN o.service where o.order_key=?1 and  o.service.task=?2")
    ...
    @Query(value = "SELECT order_running.order_id,count(*) as total,order_running.quantity,order_running.speed_up,order_running.momentum FROM history_sum join order_running  on history_sum.order_id=order_running.order_id where order_running.start_time>0 group by order_running.order_id order by start_time desc",nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE order_running set update_time=IF(total<?1,?2,update_time),total=?1,speed_up=?3,momentum=?4 where order_id=?5",nativeQuery = true)
    public void update_Total_Buff_By_OrderId(Integer total,Long update_time,Integer speed_up,Integer momentum,Long order_id);
