...
public interface OrderRunningRepository extends JpaRepository<Channel,Long> {
    @Query(value = "Select channel.channelid,channel.title,count(*) as total,maxthreads from channel left join history on history.channelid=channel.channelid and running=1 where maxthreads!=678  group by channelid order by total desc",nativeQuery = true)
    public List<OrderRunning> getOrderRunning();
    @Query(value = "Select channel.channelid,channel.title,count(*) as total,maxthreads,viewpercent,insertdate,enabled from channel left join history on history.channelid=channel.channelid and running=1 where maxthreads!=678   group by channelid",nativeQuery = true)
