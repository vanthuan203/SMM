...
public interface OrderRunningRepository extends JpaRepository<OrderRunning,Long> {
    ...
    @Query(value = "SELECT o from OrderRunning o JOIN FETCH o.service where o.service.check_count=1 and o.total>0 and o.start_time>0 and (o.update_current_time<o.update_time or (?1-o.update_current_time)/1000/60>=5)")
    ...
    @Query(value = "SELECT o.*,s.task,s.platform from order_running o left join service s on o.service_id=s.service_id where o.total>0 and (round((UNIX_TIMESTAMP()-o.update_current_time/1000)/60)>=3 or o.update_current_time<o.update_time) and s.platform=?1 and o.check_count=0 order by rand() limit 1",nativeQuery = true)
