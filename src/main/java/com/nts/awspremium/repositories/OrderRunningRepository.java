...
public interface OrderRunningRepository extends JpaRepository<OrderRunning,Long> {
    ...
    @Query(value = "SELECT o.*,s.task,s.platform from order_running o left join service s on o.service_id=s.service_id where o.total>0 and round((UNIX_TIMESTAMP()-update_current_time/1000)/60)>=3 and s.platform=?1 and o.check_count=0 order by rand() limit 1",nativeQuery = true)
