...
public interface VideoViewRepository extends JpaRepository<VideoView,Long> {
    ...
    @Query(value = "SELECT * FROM videoview where service>600 and service not in(751,752) and service not in (select service from service where (checktime=1 and mintime>=30) or live=1) and INSTR(?1,videoid)=0 and  orderid in (?2) order by rand() limit 1",nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM videoview where service<600  and service not in(151,152)  and service not in (select service from service where (checktime=1 and mintime>=30) or live=1) and INSTR(?1,videoid)=0 and  orderid in (?2) order by rand() limit 1",nativeQuery = true)
    public List<VideoView> getvideoViewVer2USTEST(String listvideo, List<String> orderid);

    @Query(value = "SELECT * FROM videoview where service in(151,152) and service not in (select service from service where (checktime=1 and mintime>=30) or live=1) and INSTR(?1,videoid)=0 and  orderid in (?2) order by rand() limit 1",nativeQuery = true)
    public List<VideoView> getvideoViewVer2USTESTNoProxy(String listvideo, List<String> orderid);

    @Query(value = "SELECT * FROM videoview where service in(751,752) and service not in (select service from service where (checktime=1 and mintime>=30) or live=1) and INSTR(?1,videoid)=0 and  orderid in (?2) order by rand() limit 1",nativeQuery = true)
