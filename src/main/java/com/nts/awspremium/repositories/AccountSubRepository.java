...
public interface AccountSubRepository extends JpaRepository<AccountSub,Long> {
    ...
    @Query(value = "Select count(*) from accountsub where username=?1 limit 1",nativeQuery = true)
    public Long checkexistAccountByUsername(String username);

    @Query(value = "Select id from accountsub where username=?1 and live=1 limit 1",nativeQuery = true)
    public Long findIdUsernameRecover(String username);
    @Query(value = "SELECT id FROM AccPremium.accountsub where username like '%nguyenthanh.com.vn' and live=1 order by rand() limit 1;",nativeQuery = true)
    public Long GetIdRecoverRand();

    @Query(value = "Select vps,count(*) from accountsub where vps in (select vps from vps where vpsoption=\"Buffh\") group by vps having count(*)>500  limit 20",nativeQuery = true)
    ...
    @Query(value = "Select proxy,proxy2 from accountsub where id=?1 limit 1",nativeQuery = true)
    public String findProxyByIdSub(Long id);

    @Query(value = "Select proxy from accountsub where id=?1 limit 1",nativeQuery = true)
    public String CheckProxyByIdSub(Long id);

    @Query(value = "Select count(*) from accountsub where username=?1 and vps=?2 limit 1",nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "update accountsub set vps='',running=0 where vps like ?1 and username not in(select username from history where vps like ?1 and running=1) limit ?2",nativeQuery = true)
    public void updatelistaccount(String vps,Integer limit);

    @Modifying
    @Transactional
    @Query(value = "INSERT INTO accountsub(username,password,recover,live,encodefinger,cookie,endtrial,endtrialstring,running,vps,date,geo) VALUES(?1,?2,?3,?4,?5,?6,0,'',?7,?8,?9,?10)",nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "INSERT INTO accountsub(username,password,recover,live,running,vps,date,timeupdateinfo) VALUES(?1,?2,?3,?4,0,'',?5,?6)",nativeQuery = true)
    public void insertAccountSub(String username,String password,String recover,Integer live,String date,Long timeupdateinfo);
    @Modifying
    @Transactional
    @Query(value = "UPDATE accountsub SET password=?1,recover=?2,live=?3,encodefinger=?4,cookie=?5,endtrial=?6,endtrialstring=?7 where username=?8",nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE accountsub SET password=?1,recover=?2,live=?3,vps=?4 where id=?5",nativeQuery = true)
    public void updateAllInfoAccSub(String password,String recover,Integer live,String vps,Long id);
    @Modifying
    @Transactional
    @Query(value = "UPDATE accountsub SET password=?1,recover=?2,live=?3,encodefinger=?4,cookie=?5,running=0 where id=?6",nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE accountsub SET running=1,vps=?2,timecheck=?3 where id=?4",nativeQuery = true)
    public void updateAccountGetByVPS(Integer running,String vps,Long timecheck ,Integer id);

    @Modifying
    @Transactional
    @Query(value = "UPDATE accountsub SET proxy=?1 where id=?2",nativeQuery = true)
    public void updateProxyAccount(String proxy,Long id);

    @Query(value = "Select * from accountsub where username=?1 limit 1",nativeQuery = true)
    public List<AccountSub> findAccountByUsername(String username);
    @Query(value = "Select * from accountsub where id=?1 limit 1",nativeQuery = true)
    public List<AccountSub> findAccountById(Long id);

    @Query(value = "Select id from accountsub where proxy='' order by rand() limit ?1",nativeQuery = true)
    ...
    @Query(value = "Select username,password,recover,oldpassword from accountsub where id=?1 limit 1",nativeQuery = true)
    public String getInfoRecover(Long id);
    @Query(value = "Select count(*) from accountsub where id=?1 and live=1 limit 1",nativeQuery = true)
    ...
    @Query(value = "SELECT id  FROM accountsub where (vps is null or vps='' or vps=' ') and running=0 and live=1 and geo=?1 order by rand()  limit 1",nativeQuery = true)
    public Long getAccountBuffh(String geo);
    @Query(value = "SELECT id  FROM accountsub where (vps is null or vps='' or vps=' ') and running=0 and live=1 and geo=?1 order by rand()  limit 1",nativeQuery = true)
    public Long getAccountView(String geo);

    @Query(value = "SELECT id  FROM accountsub where (vps is null or vps='' or vps=' ') and running=0 and live=1 and geo=?1 and INSTR(username,'@gmail.com')>0 order by rand()  limit 1",nativeQuery = true)
    public Long getAccountBuffhGmail(String geo);

    @Query(value = "SELECT id  FROM accountsub where (vps is null or vps='' or vps=' ') and running=0 and  live=1 and round((UNIX_TIMESTAMP()-timeupdateinfo/1000)/60/60/24)>=7 order by rand()  limit 1",nativeQuery = true)
    public Long getAccountSub();

    @Query(value = "SELECT id  FROM accountsub where live=0 and running=0 and round((endtrial/1000-UNIX_TIMESTAMP())/60/60/24) >=1  order by rand()  limit 1",nativeQuery = true)
    ...
    @Query(value = "SELECT id FROM accountsub where vps like ?1 and running=0 and live=1 order by rand() limit 1",nativeQuery = true)
    public Long getaccountBufhByVps(String vps);

    @Query(value = "SELECT id FROM accountsub where vps=?1 and running=0 and  live=1 and round((UNIX_TIMESTAMP()-timeupdateinfo/1000)/60/60/24)>=7 order by rand() limit 1",nativeQuery = true)
    public Long getAccountSubByVps(String vps);

    @Query(value = "SELECT count(*) FROM accountsub where live=1 and round((endtrial/1000-UNIX_TIMESTAMP())/60/60/24) >=1",nativeQuery = true)
    public Integer getCountGmails();

    @Query(value = "SELECT count(*) FROM accountsub where  round((endtrial/1000-UNIX_TIMESTAMP())/60/60/24) >=1",nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE accountsub SET proxy=?1,proxy2=?2 where id=?3",nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE accountsub SET running=1,live=1 where id=?1",nativeQuery = true)
    public Integer updateAccSubWhileCookieUpdate(Long id);

    @Modifying
    @Transactional
    @Query(value = "UPDATE accountsub SET cookie=?1,running=1,live=1 where username=?2",nativeQuery = true)
    public Integer updatecookie(String cookie,String username);

    @Modifying
    @Transactional
    @Query(value = "UPDATE accountsub SET running=0,live=1,vps='' where id=?1",nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE accountsub SET vps='',running=0 where round((UNIX_TIMESTAMP()-timecheck/1000)/60/60)>=24 and live=1 and round((endtrial/1000-UNIX_TIMESTAMP())/60/60/24) >=1 and running=1",nativeQuery = true)
    ...
    @Query(value = "SELECT geo FROM accountsub where username=?1 limit 1",nativeQuery = true)
