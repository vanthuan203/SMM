 ...
 */
    ...
    @Query(value = "select * from proxy where  round((UNIX_TIMESTAMP()-timeget/1000)/60)>=20 order by timeget asc,rand() limit 1",nativeQuery = true)
    ...
    @Query(value = "SELECT * from proxy where ipv4 like ?1 and round((UNIX_TIMESTAMP()-timeget/1000)/60)>=20 order by timeget asc,rand() limit 1;",nativeQuery = true)
