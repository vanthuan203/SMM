...
public interface ProxySettingRepository extends JpaRepository<ProxySetting,Long> {
    ...
    @Query(value = "SELECT username,password FROM proxysetting where option_proxy in(select option_setting from ipv4 where ipv4=?1) limit 1",nativeQuery = true)
    public String getUserPassByHost(String ipv4);
