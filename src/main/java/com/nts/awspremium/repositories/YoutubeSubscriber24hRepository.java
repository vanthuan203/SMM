...
public interface YoutubeSubscriber24hRepository extends JpaRepository<YoutubeSubscriber24h,String> {
    @Modifying
    @Transactional
    @Query(value = "delete from youtube_subscribe_24h where round((UNIX_TIMESTAMP()-time/1000)/60/60)>24;",nativeQuery = true)
    public Integer deleteAllByThan24h();
    @Query(value = "select count(*) from youtube_subscribe_24h where id like ?1",nativeQuery = true)
