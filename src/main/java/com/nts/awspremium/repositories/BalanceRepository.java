...
public interface BalanceRepository extends JpaRepository<Balance,Long> {
    ...
    @Query(value = "SELECT DATE(FROM_UNIXTIME((add_time / 1000), '%Y-%m-%d %H:%i:%s') + INTERVAL (7-(SELECT TIME_TO_SEC(TIMEDIFF(NOW(), UTC_TIMESTAMP)) / 3600)) hour) AS date, \n" +
            ...
            "FROM balance \n" +
            "WHERE balance < 0 and service is  not null and user in(select username from user) and FROM_UNIXTIME((add_time/1000+(7-TIME_TO_SEC(TIMEDIFF(NOW(), UTC_TIMESTAMP)) / 3600)*60*60),'%Y-%m-%d %H:%i:%s')<DATE_FORMAT(CONVERT_TZ(NOW(), @@session.time_zone, '+07:00'),'%Y-%m-%d 00-00-00') \n" +
            "GROUP BY date \n" +
            "ORDER BY date DESC \n" +
