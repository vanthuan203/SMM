...
public interface ProfileTaskRepository extends JpaRepository<ProfileTask,String> {
    ...
    @Query(value = "Select count(*) from profile_task where profile_id=?1 limit 1",nativeQuery = true)
    public Integer check_Profile_Exist(String profile_id);
    ...
    @Modifying
    @Transactional
    @Query(value = "update profile_task set enabled=0,enabled_time=0 where profile_id not in (SELECT profile_id FROM Data.account_profile where platform='tiktok' and profile_id like ?1) and enabled=1 and round((UNIX_TIMESTAMP()-enabled_time/1000)/60/60)>=6 and device_id=?2",nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE profile_task set add_proxy=1 where profile_id=?1",nativeQuery = true)
    public Integer update_Proxy_Profile_By_ProfileId(String profile_id);

    @Modifying
    @Transactional
    @Query(value = "UPDATE profile_task set clear_data=1 where profile_id=?1",nativeQuery = true)
    public Integer update_Clear_Data_Profile_By_ProfileId(String profile_id);
