...
public interface ProfileTaskRepository extends JpaRepository<ProfileTask,String> {
    ...
    @Query(value = "SELECT count(*) FROM Data.profile_task where running=1 and order_id in(select order_id from order_running where service_id in (SELECT service_id FROM service where mode='auto'));",nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE profile_task SET running=0 where round((UNIX_TIMESTAMP()-get_time/1000)/60)>15 and running=1 limit 50",nativeQuery = true)
