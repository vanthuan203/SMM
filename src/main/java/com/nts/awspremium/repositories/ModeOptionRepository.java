...
public interface ModeOptionRepository extends JpaRepository<ModeOption,Long> {
    ...
    @Query(value = "SELECT * FROM mode_option where mode=(select mode from device where device_id=(select device_id from profile_task where profile_id=?1)) and platform=?2 and task=?3 limit 1",nativeQuery = true)
    public ModeOption get_Mode_Option_By_ProfileId_And_Platform(String profile_id,String platform,String task);
