    ...
    public Map<String, Object> youtube_view(String account_id,String mode,Device device){
        ...
        try{
            ...
            if(orderRunning!=null) {
                ...
                if(youtubeView24hRepository.count_View_24h_By_DeviceId(device.getDevice_id().trim()+orderRunning.getOrder_key())>3){
                    if(ran.nextInt(100)<settingYoutube.getMax_activity_24h()){
                        return youtube_farm(account_id);
                    }else{
                        resp.put("status", false);
                ...
                if(youtubeView24hRepository.count_View_By_DeviceId_And_Hour(device.getDevice_id().trim()+orderRunning.getOrder_key(),1)>0){
                    if(ran.nextInt(100)<settingYoutube.getMax_activity_24h()){
                        return youtube_farm(account_id);
                    }else{
                        resp.put("status", false);
        ...
        try{
            ...
            if (orderRunning!=null) {
                ...
                if(dataSubscriber.getState()==1&& (System.currentTimeMillis()-dataSubscriber.getTask_time())/1000/60/60>=service.getPending_task_time()){
                    ...
                    dataSubscriberRepository.save(dataSubscriber);
                }else if(service.getPending_task() && (System.currentTimeMillis()-dataSubscriber.getTask_time())/1000/60>=60){
                    resp.put("status", false);
                    return resp;
                }
                 */
