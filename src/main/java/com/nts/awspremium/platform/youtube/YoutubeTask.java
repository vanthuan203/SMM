            ...
            if(orderRunning!=null) {
                ...
                if(service.getPending_task()){
                    if(ThreadLocalRandom.current().nextInt(100)<=3){
                        ...
                        int viewing_time= (int) (RetentionUtils.getRetentionPercentUltraHuman(orderRunning.getOrder_id(),orderRunning.getTotal(),orderRunning.getQuantity()*(1+service.getBonus()/100),service.getMin_time()/100,
                                service.getMax_time()/100,orderRunning.getThread()*orderRunning.getSpeed_up(),orderRunning.getThread()*4));
    ...
    public Map<String, Object> youtube_subscriber(String account_id,String mode,Device device){
        ...
        try{
            ...
            if (orderRunning!=null) {
                ...
                if(youtubeSubscribe24hRepository.count_Subscribe_By_DeviceId_And_OrderKey_And_Time(device.getDevice_id().trim()+orderRunning.getOrder_key(),72)>=service.getAccount_limit_24h()){
                    if(ran.nextInt(100)<settingYoutube.getMax_activity_24h()){
                        return youtube_farm(account_id);
                    }else{
                        resp.put("status", false);
    ...
    public Map<String, Object> youtube_subscriber_view(String account_id,String mode){
        ...
        try{
            ...
            if (orderRunning!=null) {
                ...
                DataSubscriber dataSubscriber=dataSubscriberRepository.get_Data_Subscriber_By_State(orderRunning.getOrder_id());
                if(dataSubscriber.getState()==1&& (System.currentTimeMillis()-dataSubscriber.getTask_time())/1000/60/60>=5){
                    dataSubscriber.setState(0);
                    dataSubscriberRepository.save(dataSubscriber);
                    ...
                    dataSubscriber.setTask_time(System.currentTimeMillis());
                    dataSubscriberRepository.save(dataSubscriber);
                }else if(service.getPending_task() && service.getLimit_task_time()==0&&(System.currentTimeMillis()-dataSubscriber.getTask_time())/1000/60>=60){
                    resp.put("status", false);
                    return resp;
                }
