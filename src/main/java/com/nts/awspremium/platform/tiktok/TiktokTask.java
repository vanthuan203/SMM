...
public class TiktokTask {
                ...
                if(service.getBonus_type()==0 || service.getBonus_list().length()==0 || service.getBonus_list_percent()==0){
                    ...
                    if(account_id.startsWith("@") && ran.nextInt(100)<service.getBonus_list_percent() ){
                        ...
                        if(bonus.equals("like")){
                            ...
                            if(modeOptionLike==null){
                                modeOptionLike=modeOptionRepository.get_Mode_Option("auto","tiktok","like");
                            }
                            ...
                            if(modeOptionShare==null){
                                modeOptionShare=modeOptionRepository.get_Mode_Option("auto","tiktok","share");
                            }
                            ...
                            if(modeOptionFavorites==null){
                                modeOptionFavorites=modeOptionRepository.get_Mode_Option("auto","tiktok","favorites");
                            }
    ...
    public Map<String, Object> tiktok_follower_view(String account_id,String mode,Device device,Integer live){
        Map<String, Object> resp = new LinkedHashMap<>();
        Map<String, Object> data = new LinkedHashMap<>();
        try{
            Random ran = new Random();
            ...
            if (orderRunning!=null) {
                ...
                if(service.getBonus_type()==0 || service.getBonus_list().length()==0 || service.getBonus_list_percent()==0){
                    ...
                    if(account_id.startsWith("@") && ran.nextInt(100)<service.getBonus_list_percent() &&live==1){
                        List<String> bonusArr = new ArrayList<>();
                        String [] bonus_list=service.getBonus_list().split(",");
                        Integer index=100;
                        for (int i=0;i<bonus_list.length;i++){
    ...
    public Map<String, Object> tiktok_view_system(String account_id,String mode,Device device,Integer live){
        Map<String, Object> resp = new LinkedHashMap<>();
        Map<String, Object> data = new LinkedHashMap<>();
        try{
            Random ran = new Random();
            ...
            if (orderRunning!=null) {
                if(account_id.startsWith("@") && ran.nextInt(100)<1 &&live==1){
                    List<String> bonusArr = new ArrayList<>();
                    ...
                    if(bonus.equals("like")){
                        ...
                        if(tikTokLike24hRepository.count_Like_24h_By_Username(account_id.trim()+"%")>=modeOptionLike.getMax_task()){
                            ...
                            String list_History_Like=tikTokLikeHistoryRepository.get_List_VideoId_By_AccountId(account_id.trim());
                            if(list_History_Like==null?true:!list_History_Like.contains(orderRunning.getVideo_tiktok().trim())){
                                data.put("bonus",bonus);
                            }else{
                                data.put("bonus","");
                            }
                        ...
                        if(tiktokShare24hRepository.count_Share_24h_By_Username(account_id.trim()+"%")>=modeOptionShare.getMax_task()){
                            ...
                            String list_History_Share=tiktokShareHistoryRepository.get_List_VideoId_By_AccountId(account_id.trim());
                            if(list_History_Share==null?true:!list_History_Share.contains(orderRunning.getVideo_tiktok().trim())){
                                data.put("bonus",bonus);
                            }else{
                                data.put("bonus","");
                            }
                        ...
                        if(tiktokFavorites24hRepository.count_Favorites_24h_By_Username(account_id.trim()+"%")>=modeOptionFavorites.getMax_task()){
                            ...
                            String list_History_Favorites=tiktokFavoritesHistoryRepository.get_List_VideoId_By_AccountId(account_id.trim());
                            if(list_History_Favorites==null?true:!list_History_Favorites.contains(orderRunning.getVideo_tiktok().trim())){
                                data.put("bonus",bonus);
                            }else{
                                data.put("bonus","");
                            }
                ...
                data.put("order_id",-1);
                data.put("account_id", account_id.trim());
                data.put("platform", "tiktok");
                data.put("task", "view");
                data.put("channel_id",orderRunning.getAccount().getAccount_id().replace("|tiktok",""));
                //data.put("user_id",orderRunning.getChannel_id());
                data.put("app", "tiktok-lite");
                data.put("task_key",orderRunning.getVideo_tiktok().trim());
                data.put("task_link","https://www.tiktok.com/"+orderRunning.getAccount().getAccount_id().replace("|tiktok","")+"/video/"+orderRunning.getVideo_tiktok().trim());
                data.put("task_key",orderRunning.getVideo_tiktok().trim());
                data.put("channel_title",orderRunning.getName());
                data.put("viewing_time",(int)(((ran.nextInt(110 - 60 + 1) + 60)/100F)*15));
                resp.put("data",data);
                return resp;
    ...
    public Map<String, Object> tiktok_view(String account_id,String mode,Device device,Integer live){
        Map<String, Object> resp = new LinkedHashMap<>();
        Map<String, Object> data = new LinkedHashMap<>();
        try{
            String mode_=mode;
