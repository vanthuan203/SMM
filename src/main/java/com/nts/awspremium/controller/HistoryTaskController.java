...
public class HistoryTaskController {
    ...
    JSONObject task_tiktok_follower(String username){
        ...
        try{
            ...
            resp.put("status", "fail");
            resp.put("fail", "logic");
            resp.put("message", "Xảy ra ngoại lệ!");
            return resp;
        }
    }
    @GetMapping(value = "test", produces = "application/hal+json;charset=utf8")
    ResponseEntity<String> test(@RequestParam(defaultValue = "") String username) {
        Random ran=new Random();
        String a=task_tiktok_follower(username.trim()).get("status").toString();
        ...
        System.out.println("===================================");
        return new ResponseEntity<String>(task_tiktok_follower(username.trim()).toJSONString(), HttpStatus.OK);
    }
