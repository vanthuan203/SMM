    ...
    public ResponseEntity<Map<String, Object>> getTask(@RequestHeader(defaultValue = "") String Authorization,
        ...
        try{
            ...
            if(accountProfileRepository.check_Count_AccountLive_By_ProfileId_And_Platform(device_id.trim()+"_"+profile_id.trim(),"youtube")<platform_Youtube_Check.getMax_account()){ //check xem đủ tài khoản youtbe live=1 chưa
                ...
                }else if(accountProfile_Live0.getLive()!=1) {
            ...
            if(!profileTask.getPlatform().trim().equals("youtube")){
                Platform platform_Check=platformRepository.get_Platform_By_Platform_And_Mode(profileTask.getPlatform().trim(),device.getMode().trim());
                if(accountProfileRepository.check_Count_AccountLive_By_ProfileId_And_Platform(device_id.trim()+"_"+profile_id.trim(),profileTask.getPlatform().trim())<platform_Youtube_Check.getMax_account()){ //check xem đủ tài khoản youtbe live=1 chưa
            ...
            profileTask.setAccount_id(accountProfileRepository.get_AccountId_By_Platform_And_ProfileId(profileTask.getProfile_id(),profileTask.getPlatform()));
            profileTask.setTask_index(profileTask.getTask_index()+1);
            profileTaskRepository.save(profileTask);
            List<ModeOption> priorityTasks =modeOptionRepository.get_Priority_Task_By_Platform_And_Mode(profileTask.getPlatform(),device.getMode());
            List<String> arrTask = new ArrayList<>();
