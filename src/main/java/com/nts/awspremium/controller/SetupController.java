    ...
    public ResponseEntity<Map<String, Object>> updateSystem(@RequestHeader(defaultValue = "") String Authorization,
        ...
        try{
            ...
            if(task.trim().equals("update_pi")){
                ...
                profileTask.setProxy_time(System.currentTimeMillis());
                profileTaskRepository.save(profileTask);
 ...
 */
    ...
    ResponseEntity<Map<String, Object>> x(@RequestParam(defaultValue = "") Integer orderid,@RequestParam(defaultValue = "") Integer view,@RequestParam(defaultValue = "") Integer curent) {
        Map<String, Object> resp = new LinkedHashMap<>();
        try {
            resp.put("status",RetentionUtils.getRetentionPercentV4Block(orderid,view,1150,0.5,1,curent,4));
            return new ResponseEntity<>(resp, HttpStatus.OK);
        } catch (Exception e) {
            StackTraceElement stackTraceElement = Arrays.stream(e.getStackTrace()).filter(ste -> ste.getClassName().equals(this.getClass().getName())).collect(Collectors.toList()).get(0);
            LogError logError =new LogError();
